...
void MgJsonDoc::Add(const char *name, const wstring & value)
{    
  MgJsonNode *node = m_tree.top();
  ...
  node->Element[StaticString(name)] = mbstr;
}

void MgJsonDoc::Add(const string &name, const wstring & value)
{    
  MgJsonNode *node = m_tree.top();
...
void MgJsonDoc::Add(const char *name, int value)
{    
  MgJsonNode *node = m_tree.top();
  node->Element[StaticString(name)] = value;
}

void MgJsonDoc::Add(const string &name, int value)
{    
  MgJsonNode *node = m_tree.top();
  node->Element[name] = value;
}
...
void MgJsonDoc::PrintStyled(string &result)
{
    result = m_styledWriter.write(m_root->Element);
}

void MgJsonDoc::PrintFast(string &result)
{
  result = m_writer.write(m_root->Element);
  
  
// Strip of \n at the end, that doesn't work well with JSONP  
