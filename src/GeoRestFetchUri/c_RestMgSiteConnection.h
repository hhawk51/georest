...
//
//  Modified by Haris Kurtagic:
//  Excluded Configuration, Service are always connected to Siste Server.
//
...
#ifndef _c_RestMgSiteConnection_h
#define _c_RestMgSiteConnection_h

// Forward declare MgConfiguration to avoid ACE header dependency
class MgConfiguration;
class REST_FETCHURI_API c_RestMgSiteConnection;
...
/// \defgroup c_RestMgSiteConnection c_RestMgSiteConnection
...
/// \endif
///
class REST_FETCHURI_API c_RestMgSiteConnection  : public MgDisposable
{
    DECLARE_CLASSNAME(c_RestMgSiteConnection)
...
PUBLISHED_API:
    ...
    /// \cond INTERNAL
    /// INTERNAL:
    /// Open should authenticate with c_RestMgSiteConnection::Authenticate which in turn uses the appropriate
    /// helper class.  A connection to the Site Server, if necessary, should be created
    /// and maintained.
    /// \endcond
    ///
    
    //static c_RestMgSiteConnection* Open(const wstring& UserName,const wstring& Password);
    static c_RestMgSiteConnection* Open(const wstring& UserName,const wstring& Password,const wstring& ServerIP,int ServerPort);
...
EXTERNAL_API:
    ...
    /// \endcond
    ///
    c_RestMgSiteConnection(CREFSTRING url);
...
INTERNAL_API:
    ...
    /// Nothing
    ///
    static void SetCurrentConnection(c_RestMgSiteConnection* siteConnection);
    ...
    ///
    static c_RestMgSiteConnection* GetCurrentConnection();
...
CLASS_ID:
    ...
    static const INT32 m_cls_id = 0; // MapGuide_Service_SiteConnection;
};
/// \}
