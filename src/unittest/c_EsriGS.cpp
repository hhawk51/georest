...
CPPUNIT_TEST_SUITE_REGISTRATION(c_EsriGS);
...
void c_EsriGS::TestCatalogDocument()
{
try
{
  ...
  HTTPRequest request(HTTPRequest::HTTP_GET, "/rest/Esri.GS");
  ...
  c_CfgEsriGS_Folder folder;
  
  folder.ReadFromJson(ostr.str());
  
  c_CfgEsriGS_Folder * sheboygan_folder = folder.FindSubFolder(L"Sheboygan");
  if( !sheboygan_folder )
  {
    CPPUNIT_FAIL("No Sheboygan folder in response!");
  }
}
...
void c_EsriGS::TestSubFolderDocument()
{
  try
  {
    c_RestConfig * restcfg = c_RestConfig::GetInstance();
    ...
    HTTPClientSession s("localhost", 99);
    HTTPRequest request(HTTPRequest::HTTP_GET, "/rest/Esri.GS/Sheboygan");
    Poco::Timespan timeout(2,0);
    s.setTimeout(timeout);
    s.sendRequest(request);
    ...
    c_CfgEsriGS_Folder * sheboygan_subfolder = folder.FindSubFolder(L"Sheboygan_Sub1");
    if( !sheboygan_subfolder )
    {
      CPPUNIT_FAIL("No Sheboygan folder in response!");
    }
    
    // and Sheboygan folder is shouldn't be there
    c_CfgEsriGS_Folder * sheboygan_folder = folder.FindSubFolder(L"Sheboygan");
    if( sheboygan_folder )
    {
      CPPUNIT_FAIL("Sheboygan folder can't be there!");
    }
    
    c_CfgEsriGS_Folder * some = folder.FindSubFolder(L"some");
    if( some )
    {
      CPPUNIT_FAIL("There shouldn't be some subfolder!");
    }
  }
  catch(c_ExceptionRestConfig& exc)  
  {
...
void c_EsriGS::TestSubFolderDocument2()
{
  try
  {
    c_RestConfig * restcfg = c_RestConfig::GetInstance();
    ...
    HTTPClientSession s("localhost", 99);
    HTTPRequest request(HTTPRequest::HTTP_GET, "/rest/Esri.GS/Sheboygan/Sheboygan_Sub1");
    Poco::Timespan timeout(2,0);
    s.setTimeout(timeout);
    s.sendRequest(request);
...
}//end of TestSubFolderDocument2


void c_EsriGS::TestOdataCollection()
{
  c_RestConfig * restcfg = c_RestConfig::GetInstance();
  restcfg->Clear();
  restcfg->ReadFromString(EsriGSSample,"/templates/");
...
void c_EsriGS::TestParseKeyValueString()
{
...
void c_EsriGS::TestParseOdataLiteralValues()
{
//
// Test Byte
//
...
}// end of c_EsriGS::TestParseOdataLiteralValues

void c_EsriGS::TestParseOdataFilter()
{
...
void c_EsriGS::TestOdataOneProperty()
{
  c_RestConfig * restcfg = c_RestConfig::GetInstance();
  restcfg->Clear();
  restcfg->ReadFromString(EsriGSSample,"/templates/");
...
}//end of  c_EsriGS::TestOdataOneProperty

void c_EsriGS::TestOdataCollection_Json()
{
  c_RestConfig * restcfg = c_RestConfig::GetInstance();
  restcfg->Clear();
  restcfg->ReadFromString(EsriGSSample,"/templates/");
...
}//end of c_EsriGS::TestOdataCollection_Json

void c_EsriGS::TestOrderBy()
{
  c_RestConfig * restcfg = c_RestConfig::GetInstance();
  restcfg->Clear();
  restcfg->ReadFromString(EsriGSSample,"/templates/");
...
}//end of c_EsriGS::TestOrderBy

void c_EsriGS::JustMyTest()
{
  int a = sizeof(long);
